...
BEGIN
    IF TG_TABLE_NAME = 'intercambio' THEN
        IF NEW.estado_inter = 'satisfactorio' THEN
...
FOR EACH ROW
WHEN (OLD.estado_inter IS DISTINCT FROM NEW.estado_inter AND NEW.estado_inter = 'satisfactorio')
...
WHERE NOT EXISTS (SELECT 1 FROM logro WHERE titulo_logro = 'Eco-Amigable')`
    ...
    try {
        ...
        console.log("Successfully applied SQL triggers.");
    } catch (e) {
