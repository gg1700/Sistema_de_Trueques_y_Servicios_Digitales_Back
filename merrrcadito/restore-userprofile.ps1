...
# Replace the template variables with actual braces for interpolation
...
$content = $content.Replace("{p.cod_pub ?? p.id ?? 0}", "`${p.cod_pub ?? p.id ?? 0}")
$content = $content.Replace("{codUs}", "`${codUs}")
$content = $content.Replace("{Buffer.from(s.foto_serv).toString('base64')}", "`${Buffer.from(s.foto_serv).toString('base64')}")
$content = $content.Replace("{encodeURIComponent(", "`${encodeURIComponent(")
$content = $content.Replace("{user.cod_us}", "`${user.cod_us}")
$content = $content.Replace("{codProd}", "`${codProd}")
$content = $content.Replace("{err.message}", "`${err.message}")
$content = $content.Replace("{user.nom_us}", "`${user.nom_us}")
$content = $content.Replace("{user.ap_pat_us}", "`${user.ap_pat_us}")
$content = $content.Replace("{user.ap_mat_us ?? ""}", "`${user.ap_mat_us ?? ""}")
$content = $content.Replace("{styles.publishTab}", "`${styles.publishTab}")
$content = $content.Replace("{publishType === ", "`${publishType === ")
$content = $content.Replace("{styles.sideMenuLink}", "`${styles.sideMenuLink}")
$content = $content.Replace("{isActive ?", "`${isActive ?")
$content = $content.Replace("{styles.tab}", "`${styles.tab}")
$content = $content.Replace("{activeTab === ", "`${activeTab === ")

Set-Content -Path $path -Value $content -Encoding UTF8
Write-Host "UserProfile.tsx restored and fixed."
