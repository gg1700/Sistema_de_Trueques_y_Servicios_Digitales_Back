...
export async function register_product(
  cod_subcat_prod: string,
  attributes: Partial<ProductInfo>
) {
  try {
    if (attributes.estado_prod == null) {
      attributes.estado_prod = 'disponible';
    }
    if (attributes.calidad_prod == null) {
      attributes.calidad_prod = 'nuevo';
    }
    const result: any = await prisma.$queryRaw`
      SELECT * FROM sp_registrarproducto(
