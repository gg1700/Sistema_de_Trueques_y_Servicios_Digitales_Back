...
export async function register_transaction(cod_us_origen: string, attributes: Partial<TransactionInfo>) {
    try {
        ...
        return await prisma.$transaction(async (tx) => {
            const exists_origin = await tx.$queryRaw`
            ...
            if (attributes.cod_pub != null && attributes.cod_evento == null && attributes.id_token == null) {
                // Es una publicación
                const exists_pub = await tx.$queryRaw`
                ...
                if (!result_pub) {
                    ...
                    const saldo_us_origen: any[] = await tx.$queryRaw`
                    ...
                    // Obtener precio con descuento (si aplica promoción)
                    const priceData = await PromotionService.getPublicationPrice(parseInt(attributes.cod_pub));
                ...
                // Es un evento
                const exists_event = await tx.$queryRaw`
                ...
                if (!result_event) {
                    ...
                    const [wallet_origin] = saldo_us_origen;
                    const { saldo_actual } = wallet_origin;
                    const event_cost: any[] = await tx.$queryRaw`
                ...
                const exists_token = await tx.$queryRaw`
                ...
                if (!result_token) {
                    ...
                    const token_package: any[] = await tx.$queryRaw`
            ...
            // Registrar transacción DENTRO DE LA TRANSACCIÓN
            console.log('[TRANSACTION] Registrando transacción en la BD...');
            ...
            console.log(`[TRANSACTION] Transacción registrada con código: ${cod_trans}`);

            // Registrar pago en escrow DENTRO DE LA TRANSACCIÓN
            await tx.$queryRaw`
            ...
            console.log('[TRANSACTION] Pago escrow registrado');

            // Registrar en bitácora DENTRO DE LA TRANSACCIÓN
            // CRÍTICO: Pasar el cliente transaccional 'tx' para que pueda ver el cod_trans recién creado
            const binnacle_result = await BinnacleService.register_transaction_binnacle(cod_trans, tx);
            if (!binnacle_result.success) {
                throw new Error('Error al registrar en bitácora: ' + binnacle_result.message);
            }

            console.log('[TRANSACTION] Bitácora registrada exitosamente');

            return { success: true, message: "Transacción registrada correctamente" };
        });
    } catch (err) {
        console.error('[TRANSACTION ERROR] Detalles completos del error:', {
