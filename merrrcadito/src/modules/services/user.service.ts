...
export async function get_user_total_sales(cod_us: number) {
    try {
        const result: any[] = await prisma.$queryRaw`
            SELECT COUNT(*) as total_ventas
            FROM transaccion t
            INNER JOIN escrow e ON t.cod_trans = e.cod_trans
            WHERE t.cod_us_destino = ${cod_us}
            AND t.estado_trans = 'satisfactorio'
            AND e.estado_escrow = 'liberado'
    `;

        const totalVentas = Number(result[0]?.total_ventas || 0);
        return totalVentas;
    } catch (err) {
        console.error('Error en get_user_total_sales:', err);
        throw new Error((err as Error).message);
    }
}

// OBTENER TOTAL DE INGRESOS DE UN USUARIO (SUMA DE CV)
export async function get_user_total_income(cod_us: number) {
    try {
        const result: any[] = await prisma.$queryRaw`
            SELECT COALESCE(SUM(COALESCE(prod.precio_prod, serv.precio_serv, 0)), 0) as total_ingresos
            FROM transaccion t
            INNER JOIN escrow e ON t.cod_trans = e.cod_trans
            INNER JOIN publicacion pub ON t.cod_pub = pub.cod_pub
            ...
            AND e.estado_escrow = 'liberado'
    `;
        const totalIngresos = Number(result[0]?.total_ingresos || 0);
        return totalIngresos;
    } catch (err) {
        console.error('Error en get_user_total_income:', err);
        throw new Error((err as Error).message);
    }
}

// OBTENER VENTAS POR CATEGORIA DE UN USUARIO
export async function get_user_sales_by_category(cod_us: number, month?: string, year?: string) {
    try {
        // Construir la condición de fecha dinámicamente
        let dateFilter = Prisma.sql``;

        console.log('get_user_sales_by_category service args:', { cod_us, month, year });

        if (month && year) {
            dateFilter = Prisma.sql`AND EXTRACT(MONTH FROM t.fecha_trans) = ${parseInt(month)} AND EXTRACT(YEAR FROM t.fecha_trans) = ${parseInt(year)} `;
        } else if (month) {
            dateFilter = Prisma.sql`AND EXTRACT(MONTH FROM t.fecha_trans) = ${parseInt(month)} `;
        } else if (year) {
            dateFilter = Prisma.sql`AND EXTRACT(YEAR FROM t.fecha_trans) = ${parseInt(year)} `;
        }

        console.log('dateFilter generated:', dateFilter);

        const result: any[] = await prisma.$queryRaw`
            SELECT c.nom_cat, COUNT(*) as cantidad
            FROM transaccion t
            INNER JOIN escrow e ON t.cod_trans = e.cod_trans
            INNER JOIN publicacion pub ON t.cod_pub = pub.cod_pub
            INNER JOIN publicacion_producto pp ON pub.cod_pub = pp.cod_pub
            INNER JOIN producto prod ON pp.cod_prod = prod.cod_prod
            INNER JOIN subcategoria_producto sub ON prod.cod_subcat_prod = sub.cod_subcat_prod
            INNER JOIN categoria c ON sub.cod_cat = c.cod_cat
            WHERE t.cod_us_destino = ${cod_us}
            AND t.estado_trans = 'satisfactorio'
            AND e.estado_escrow = 'liberado'
            ${dateFilter}
    ...
    `;
        ...
        return result.map(item => ({
            categoria: item.nom_cat,
            cantidad: Number(item.cantidad)
        }));
    } catch (err) {
        console.error('Error en get_user_sales_by_category:', err);
        throw new Error((err as Error).message);
    }
}

// OBTENER CANTIDAD DE VENTAS POR MES (FILTRADO POR AÑO)
export async function get_user_sales_by_month(cod_us: number, year?: string) {
    try {
        let dateFilter = Prisma.sql``;

        console.log('get_user_sales_by_month service args:', { cod_us, year });
        ...
        const result: any[] = await prisma.$queryRaw`
            ...
            AND e.estado_escrow = 'liberado'
            ${dateFilter}
            GROUP BY EXTRACT(MONTH FROM t.fecha_trans)
            ORDER BY mes
        `;

        // Convertir BigInt a Number y asegurar que tenemos todos los meses
        const monthsData = new Array(12).fill(0).map((_, index) => ({
            mes: index + 1,
            cantidad: 0
        }));
        ...
        console.error('Error en get_user_sales_by_month:', err);
        throw new Error((err as Error).message);
    }
}

// OBTENER INGRESOS TOTALES POR MES (FILTRADO POR AÑO)
export async function get_user_income_by_month(cod_us: number, year?: string) {
    try {
        let dateFilter = Prisma.sql``;

        console.log('get_user_income_by_month service args:', { cod_us, year });
        ...
        const result: any[] = await prisma.$queryRaw`
            SELECT 
                EXTRACT(MONTH FROM t.fecha_trans) as mes,
                COALESCE(SUM(COALESCE(prod.precio_prod, serv.precio_serv, 0)), 0) as ingresos
            FROM transaccion t
            INNER JOIN escrow e ON t.cod_trans = e.cod_trans
            INNER JOIN publicacion pub ON t.cod_pub = pub.cod_pub
            LEFT JOIN publicacion_producto pp ON pub.cod_pub = pp.cod_pub
        ...
        console.error('Error en get_user_income_by_month:', err);
        throw new Error((err as Error).message);
    }
}